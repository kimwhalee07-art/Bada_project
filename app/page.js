...
// 다국어 사전 (ko, en, zh, vi) - 한패스 제거 및 마이페이지 다국어 반영
const translations = {
  ko: {
    nav: { plans: '유심 요금제', internet: '인터넷·TV', rental: '가전·렌탈', stores: '수령매장', faq: 'FAQ', login: '로그인', apply: '신청하기', logout: '로그아웃', mypage: '마이페이지' },
    hero: { badge: '⚡ 외국인을 위한 한국 생활 원스톱 통신 & 가전', t1: '선불유심부터', t2: '초고속 인터넷 & 가전렌탈', t3: '까지', desc: '여권/외국인등록증 5분 개통 알뜰폰·통신사 유심, 백메가 제휴 최대 현금 사은품 인터넷 설치, 부담 없는 가전 렌탈을 바다에서 한 번에 누리세요.', bP: '유심 요금제 보기 ↓', bI: '인터넷 사은품 확인 🎁', bC: '1:1 직원 상담 💬' },
  ...
  en: {
    nav: { plans: 'SIM Plans', internet: 'Internet·TV', rental: 'Rental', stores: 'Stores', faq: 'FAQ', login: 'Login', apply: 'Apply Now', logout: 'Logout', mypage: 'My Page' },
    hero: { badge: '⚡ One-stop Telecom & Appliances for Foreigners in Korea', t1: 'From Prepaid SIM to', t2: 'High-speed Internet & Rentals', t3: '', desc: 'Activate Budget Phone (MVNO) or Major Carrier SIM in 5 mins with Passport/ARC, receive maximum cash gifts for internet, and get home appliances.', bP: 'View SIM Plans ↓', bI: 'Internet Cash Gift 🎁', bC: '1:1 Live Support 💬' },
  ...
  zh: {
    nav: { plans: '电话卡套餐', internet: '宽带·电视', rental: '家电租赁', stores: '自提门店', faq: '常见问题', login: '登录', apply: '立即申请', logout: '退出登录', mypage: '个人中心' },
    hero: { badge: '⚡ 专为在韩外国人打造的一站式通信与家电服务', t1: '从预付费电话卡到', t2: '高速宽带与家电租赁', t3: '全搞定', desc: '支持护照/登录证5分钟办理廉价卡(MVNO)与三大运营商SIM卡，宽带享最高现金补贴，家电轻松月租。', bP: '查看电话卡套餐 ↓', bI: '查看宽带补贴 🎁', bC: '人工1:1在线咨询 💬' },
  ...
  vi: {
    nav: { plans: 'Gói Cước SIM', internet: 'Internet·TV', rental: 'Thuê Thiết Bị', stores: 'Cửa Hàng', faq: 'FAQ', login: 'Đăng Nhập', apply: 'Đăng Ký', logout: 'Đăng Xuất', mypage: 'Trang Của Tôi' },
    hero: { badge: '⚡ Dịch vụ Viễn thông & Gia dụng Trọn gói tại Hàn Quốc', t1: 'Từ SIM Trả Trước đến', t2: 'Internet & Thuê Thiết Bị', t3: '', desc: 'Kích hoạt SIM giá rẻ (MVNO) hoặc 3 nhà mạng lớn trong 5 phút bằng Hộ chiếu/ARC, nhận tiền mặt quà tặng Internet tối đa.', bP: 'Xem Gói Cước ↓', bI: 'Quà Tặng Internet 🎁', bC: 'Tư Vấn 1:1 💬' },
...
export default function BadaPage() {
  ...
  const [registeredUsers, setRegisteredUsers] = useState([
    { userCode: '0000_0000_0001', username: 'mimimimi', name: '외국인 고객', email: 'test@badahub.co.kr', password: '123', phone: '010-9988-7766', address: '충남 천안시 동남구 대흥로 123', bankName: '국민은행', bankAccount: '123-456-789012', bankHolder: '외국인 고객' }
  ]);
  ...
  const [assignedStaff, setAssignedStaff] = useState({ name: '김상담 주임', dept: '상담 1팀 (다국어 전담)' });
    ...
    if (savedLogin) {
      try {
        ...
        setMyProfile({
          ...
          phone: u.phone || '',
          address: u.address || '',
          bankName: u.bankName || '국민은행',
          bankAccount: u.bankAccount || '',
          bankHolder: u.bankHolder || u.name || ''
        });
        setApplyForm(prev => ({ ...prev, name: u.name, email: u.email, phone: u.phone || '' }));
      } catch (e) {}
    }
  ...
  const handleGoogleLogin = () => {
    if (!GOOGLE_CLIENT_ID) {
      alert('⚠️ 구글 간편 로그인은 아직 준비 중입니다.\n\n(클라이언트 ID 발급 후 연동될 예정입니다. 지금은 아래의 일반 로그인/회원가입을 이용해 주세요!)');
      return;
    }
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (rooms[sessionId]) {
        ...
        if (rooms[sessionId].assignedStaffName) {
          setAssignedStaff({ name: rooms[sessionId].assignedStaffName, dept: '전담 상담팀' });
        }
      }
    } catch (e) {}
  ...
  const handleLoginSubmit = (e) => {
    ...
    const sessionUser = {
      ...
      bankName: found.bankName || '국민은행',
      bankAccount: found.bankAccount || '',
      bankHolder: found.bankHolder || found.name
    };
    setCurrentUser(sessionUser);
    setMyProfile(sessionUser);
    localStorage.setItem('bada_current_login', JSON.stringify(sessionUser));
    setApplyForm(prev => ({ ...prev, name: sessionUser.name, email: sessionUser.email, phone: sessionUser.phone }));
    setShowLoginModal(false);
    alert(`반갑습니다, ${sessionUser.name}님!`);
  };
  ...
  const handleRegisterSubmit = (e) => {
    ...
    const newUser = {
      ...
      phone: '', address: '', bankName: '국민은행', bankAccount: '', bankHolder: regForm.name || cleanId
    };
    ...
    setShowLoginModal(false);
    alert(`회원가입이 완료되었습니다! 반갑습니다, ${newUser.name}님.`);
  };
  ...
  const handleSaveMyProfile = (e) => {
    ...
    if (!myProfile.name.trim()) return alert('성함을 입력해 주세요.');

    const updatedUser = {
      ...currentUser,
      name: myProfile.name.trim(),
      phone: myProfile.phone.trim(),
      address: myProfile.address.trim(),
      bankName: myProfile.bankName,
      bankAccount: myProfile.bankAccount.trim(),
      bankHolder: myProfile.bankHolder.trim() || myProfile.name.trim()
    };

    setCurrentUser(updatedUser);
    localStorage.setItem('bada_current_login', JSON.stringify(updatedUser));

    const updatedDb = registeredUsers.map(u => u.username === updatedUser.username ? { ...u, ...updatedUser } : u);
    setRegisteredUsers(updatedDb);
    localStorage.setItem('bada_user_db', JSON.stringify(updatedDb));

    setApplyForm(p => ({ ...p, name: updatedUser.name, phone: updatedUser.phone }));
    alert('✅ 이름 및 마이페이지 정보가 성공적으로 저장되었습니다!');
  };
  ...
  const handleApplySubmit = (e) => {
    ...
    const newOrder = {
      ...
      detail: `[${applyForm.telecomCategory === 'mvno' ? '알뜰폰' : '통신 3사'} · ${applyForm.carrier}] ${applyForm.plan} (${applyForm.deliveryMethod === 'store' ? '천안 본점 픽업' : applyForm.deliveryMethod === 'delivery' ? '무료 택배 배송' : 'eSIM 즉시 발급'})`,
      time: '방금 전'
    };
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#f8fafc', color: '#0f172a', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif' }}>
      ...
      <style>{`
        ...
        /* 데스크탑 네비게이션: 중앙 1줄 배치 (한패스 제거로 더욱 널찍함) */
        .desktop-nav { display: flex; align-items: center; gap: clamp(10px, 1.8vw, 24px); font-size: 13px; font-weight: 600; white-space: nowrap; flex-shrink: 0; }
        ...
        @media (max-width: 820px) {
          ...
          .mobile-tab-item:active { background: #e0f2fe; color: #0284c7; }
          .mobile-tab-item.highlight { background: #e0f2fe; color: #0369a1; border-color: #bae6fd; font-weight: bold; }
        }
      `}</style>

      {/* 1. 상단 네비게이션 헤더 (한패스 완전 제거, 노트북 1줄 & 폰 좌우 슬라이드) */}
        ...
        <div className="header-main-row" style={{ maxWidth: '1280px', margin: '0 auto', padding: '0 16px', height: '64px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '8px', flexWrap: 'nowrap' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flexShrink: 0, whiteSpace: 'nowrap' }}>
            ...
            {!currentUser ? (
              ...
              <button onClick={() => setShowMyPageModal(true)} style={{ background: '#e0f2fe', border: '1px solid #bae6fd', padding: '5px 10px', borderRadius: '16px', fontSize: '12px', fontWeight: 'bold', color: '#0369a1', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '4px', whiteSpace: 'nowrap', flexShrink: 0 }}>
                <span>👤</span> {currentUser.name || currentUser.username}님 (마이페이지)
              </button>
            )}
      ...
      {/* 12. [신규 완성] 고객 마이페이지 (My Page) 모달 */}
      {showMyPageModal && currentUser && (
        <div className="modal-bg">
          <div className="modal-box" style={{ maxWidth: '440px', padding: '24px' }}>
            {/* 마이페이지 상단 헤더 */}
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', borderBottom: '1px solid #e2e8f0', paddingBottom: '12px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                <div style={{ width: '32px', height: '32px', borderRadius: '16px', backgroundColor: '#0284c7', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold' }}>
                  {currentUser.name ? currentUser.name.charAt(0).toUpperCase() : 'U'}
                </div>
                <div>
                  <h3 style={{ margin: 0, fontSize: '16px', fontWeight: 'bold', color: '#0f172a' }}>{currentUser.name || currentUser.username} 님의 마이페이지</h3>
                  <span style={{ fontSize: '11px', color: '#64748b' }}>고객코드: {currentUser.userCode}</span>
                </div>
              </div>
              <button onClick={() => setShowMyPageModal(false)} style={{ background: 'none', border: 'none', fontSize: '22px', cursor: 'pointer', color: '#94a3b8' }}>✕</button>
            </div>

            {/* 1:1 전담 직원 연결 카드 */}
            <div style={{ backgroundColor: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: '10px', padding: '11px 14px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '18px' }}>
              <div>
                <div style={{ fontSize: '11px', color: '#1e40af', fontWeight: 'bold' }}>👤 고객님 전담 상담 직원</div>
                <div style={{ fontSize: '13px', fontWeight: '800', color: '#1e3a8a', marginTop: '2px' }}>{assignedStaff.name} <span style={{ fontSize: '11px', fontWeight: 'normal', color: '#3b82f6' }}>({assignedStaff.dept})</span></div>
              </div>
              <button onClick={() => { setShowMyPageModal(false); setShowChat(true); setChatMode('staff_chat'); }} className="btn-pri" style={{ padding: '6px 12px', fontSize: '12px' }}>
                💬 1:1 직통 상담
              </button>
            </div>

            {/* 내 실시간 신청 현황 카드 */}
            <div style={{ backgroundColor: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '10px', padding: '12px', marginBottom: '18px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                <span style={{ fontSize: '12px', fontWeight: 'bold', color: '#334155' }}>📦 실시간 신청/개통 현황</span>
                <span style={{ fontSize: '11px', color: '#0284c7', backgroundColor: '#e0f2fe', padding: '2px 8px', borderRadius: '10px', fontWeight: 'bold' }}>🚚 배송/개통 준비 중</span>
              </div>
              <div style={{ fontSize: '13px', fontWeight: 'bold', color: '#0f172a' }}>
                {applyForm.plan ? `[${applyForm.carrier}] ${applyForm.plan}` : '데이터 무제한 30일 (알뜰폰 KT망)'}
              </div>
              <div style={{ fontSize: '11px', color: '#64748b', marginTop: '4px' }}>
                • 수령 방식: {applyForm.deliveryMethod === 'store' ? '🏬 천안 본점 방문 픽업' : applyForm.deliveryMethod === 'delivery' ? '📦 전국 무료 택배 (우체국/CJ)' : '📲 eSIM 즉시 발급'}<br />
                • 진행 상태: 서류 검토 완료 ➔ 5분 즉시 개통 처리 중
              </div>
            </div>

            {/* 프로필 & 계좌 정보 수정 폼 */}
            <form onSubmit={handleSaveMyProfile} style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              <div style={{ borderTop: '1px solid #f1f5f9', paddingTop: '10px' }}>
                <strong style={{ fontSize: '13px', color: '#0f172a', display: 'block', marginBottom: '8px' }}>✏️ 내 프로필 정보 (이름/연락처/배송지)</strong>
                
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  <div>
                    <label className="bada-lbl">성함 (여권/등록증 상 영문 또는 한글)</label>
                    <input type="text" value={myProfile.name} onChange={e => setMyProfile({ ...myProfile, name: e.target.value })} placeholder="예: NGUYEN VAN A" className="bada-inp" required />
                  </div>
                  <div>
                    <label className="bada-lbl">연락처 (전화번호)</label>
                    <input type="tel" value={myProfile.phone} onChange={e => setMyProfile({ ...myProfile, phone: e.target.value })} placeholder="예: 010-1234-5678" className="bada-inp" />
                  </div>
                  <div>
                    <label className="bada-lbl">택배 기본 배송지 주소 (원룸/기숙사)</label>
                    <input type="text" value={myProfile.address} onChange={e => setMyProfile({ ...myProfile, address: e.target.value })} placeholder="충남 천안시 동남구 대흥로 ..." className="bada-inp" />
                  </div>
                </div>
              </div>

              {/* 사은품 입금 계좌 설정 (인터넷+TV 신청 고객용) */}
              <div style={{ borderTop: '1px solid #f1f5f9', paddingTop: '10px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
                  <strong style={{ fontSize: '13px', color: '#0f172a' }}>💳 사은품 입금받을 계좌 (인터넷 현금 지원용)</strong>
                  <span style={{ fontSize: '10px', color: '#059669', fontWeight: 'bold' }}>최대 47만원 지급</span>
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: '100px 1fr', gap: '6px' }}>
                  <select value={myProfile.bankName} onChange={e => setMyProfile({ ...myProfile, bankName: e.target.value })} className="bada-inp">
                  ...
                  <input type="text" value={myProfile.bankAccount} onChange={e => setMyProfile({ ...myProfile, bankAccount: e.target.value })} placeholder="계좌번호 입력 ( - 제외)" className="bada-inp" />
                </div>
                <input type="text" value={myProfile.bankHolder} onChange={e => setMyProfile({ ...myProfile, bankHolder: e.target.value })} placeholder="예금주 성함" className="bada-inp" style={{ marginTop: '6px' }} />
              </div>

              <button type="submit" className="btn-pri" style={{ marginTop: '6px', padding: '11px', width: '100%' }}>
                💾 변경사항 저장하기
              </button>
            </form>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '18px', borderTop: '1px solid #f1f5f9', paddingTop: '12px' }}>
              <button onClick={handleLogout} className="btn-sec" style={{ fontSize: '12px', padding: '6px 14px' }}>
                로그아웃
              </button>
              <button onClick={handleDeleteAccount} style={{ background: 'none', border: 'none', color: '#ef4444', fontSize: '11px', cursor: 'pointer', textDecoration: 'underline' }}>
                회원 탈퇴 (데이터 삭제)
              </button>
            </div>
          </div>
